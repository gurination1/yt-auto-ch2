...
def generate_script(topic: dict, format_type: str) -> dict:
    ...
    if format_type == "short":
        ...
        hook_formatted = hook_pattern.format(
            subject=topic.get("topic", "nature"),
            thing=topic.get("topic", "nature"),
            seconds="30",
            topic=topic.get("topic", "nature"),
            event="A discovery"
        )
        lang_instruction = ""
    ...
    if script is None:
        ...
        if niche == "engineering":
            ...
            script = {
                ...
                "tags": ["nature", "physics", "technology", "quantum", "universe", "didyouknow"],
                "category_id": "15",
                "segments": [
                    {
                        "id": 1,
